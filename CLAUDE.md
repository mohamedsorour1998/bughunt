...
src/
  ...
  lib/
    ...
    redis.ts         # enqueuePlayer, findMatch, rateLimitCheck, publishGameEvent
...
2. Next caller with compatible ELO → dequeue both → `createGame` → returns `{status:"active", gameId}`
